...
(function () {
  ...
  function initStickyHeader() {
    ...
    // The header sticks at --safe-top, so it is stuck the moment the hero's
    // top edge reaches (safe-top + header height) from the viewport top.
    // Firing the plate right there, rather than when the sentinel leaves
    // the viewport entirely, closes the window where a stuck header sat
    // transparent over the iPhone status bar with content passing under
    // the clock. env() is measured through a probe: getComputedStyle hands
    // back the unresolved env() token for a custom property.
    ...
    var safeTop = probe.offsetHeight || 0;
    probe.remove();
    var stuckAt = safeTop + header.offsetHeight;

    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        header.classList.toggle("is-scrolled", !entry.isIntersecting);
      });
    }, { rootMargin: "-" + stuckAt + "px 0px 0px 0px" });
  ...
  /* ---------- Reading progress ----------
   * One custom property, consumed by the header rule and the ring around the
   * back-to-top button. Written on those two elements — NOT on <html>: a
   * custom property changing on the root invalidates style for every
   * element in the document on every scroll frame. tokens.css keeps the
   * :root default of 0 as the no-JS fallback.
   */
  function initScrollProgress() {
    var root = document.documentElement;
    var targets = Array.prototype.slice.call(document.querySelectorAll("#header, .back-to-top"));
    ...
    function write() {
      ...
      var v = Math.min(1, Math.max(0, p)).toFixed(4);
      for (var i = 0; i < targets.length; i++) targets[i].style.setProperty("--scroll-progress", v);
    }
    function request() {
      if (ticking) return;
      ticking = true;
  ...
  /* ---------- Hero carousel: autoplay, prev/next, dots, swipe, pause ----------
   * ONE source of truth: `current`. render() derives everything visible from
   * it — the track's --hero-x transform (which photograph is on screen) and
   * .is-active (which slide's copy is shown) are written in the same call, so
   * they cannot disagree. The previous build let native scrolling decide the
   * photograph (scrollTo + scroll-snap) and JS decide the copy; a smooth
   * scroll interrupted by a hidden tab, an iOS URL-bar resize or focus moving
   * into an off-screen slide left the two out of step: a photo with no text,
   * a Next that only changed the text, the old photo under the new headline.
   *
   * Autoplay is one setTimeout per slide, never an interval. Holds are keyed
   * by reason in a Set (hover, focus, hidden, offscreen, user, drag), so an
  ...
  function initHeroCarousel() {
    ...
    var dotsWrap = hero && hero.querySelector(".hero-dots");
    var controls = hero && hero.querySelector(".hero-controls");
    var prevBtn = hero && hero.querySelector(".hero-arrow.prev");
    var nextBtn = hero && hero.querySelector(".hero-arrow.next");
    var playBtn = hero && hero.querySelector(".hero-playpause");
    if (!hero || !track || !slides.length) return;

    var n = slides.length;
    var current = 0;
    var leaving = -1;        // slide on its way out, keeps its copy for --hero-ms
    var leavingTimer = null;
    var holds = new Set();   // reasons autoplay is currently held
    var timer = null;
    var deadline = 0;        // when the running timer will fire
    var remaining = SLIDE_MS; // what a resume should arm
    var dots = [];

    // Slide-over duration comes from the CSS token so the two never drift;
    // reduced motion zeroes it there.
    var heroMs = parseFloat(window.getComputedStyle(hero).getPropertyValue("--hero-ms")) || 0;
    // …and the autoplay interval goes the other way, so the dot fill and the
    // timer share one clock.
    hero.style.setProperty("--hero-slide-ms", SLIDE_MS + "ms");
    ...
    /* --- State → DOM. Idempotent; the only place slide/dot state is written. --- */
    function render() {
      track.style.setProperty("--hero-x", (-current * 100) + "%");
      slides.forEach(function (s, i) {
        var active = i === current;
        s.classList.toggle("is-active", active);
        s.classList.toggle("is-leaving", !active && i === leaving);
        // Inactive slides are out of the accessibility tree and the focus
        // order (visibility:hidden in the CSS does the same for browsers
        // without inert), so Tab can never land on an off-screen link.
        if (active) {
    ...
    function renderPlayState() {
      var paused = holds.size > 0;
      hero.classList.toggle("hero-paused", paused);
      // Announce slide changes only while the visitor has stopped the show
      // themselves (hover / focus / pause) — APG carousel pattern.
      track.setAttribute("aria-live", paused ? "polite" : "off");
      if (playBtn) {
        var userPaused = holds.has("user");
    ...
    function goTo(index, user) {
      ...
      if (target === current) {
        if (user) arm(SLIDE_MS);
        return;
      }
      leaving = current;
      current = target;
      render();
      // The outgoing slide keeps its copy and its Ken Burns frame until the
      // slide-over is done, then drops back to the hidden state unseen.
    ...
    /* --- Autoplay --- */
    function canPlay() { return !prefersReducedMotion && holds.size === 0; }

    function clearTimer() {
      if (timer) window.clearTimeout(timer);
      timer = null;
    }
    // Always records `remaining` so a later release arms the right amount
    // even when this call could not start a timer (held at the time).
    function arm(ms) {
      clearTimer();
      remaining = ms;
      if (!canPlay()) return;
    ...
    function release(reason) {
      ...
      renderPlayState();
      // Resume where the dot fill paused; a sliver of time left just rolls
      // into a whole new interval so the change never fires the instant the
      // pointer leaves.
      if (holds.size === 0) arm(remaining > 250 ? remaining : SLIDE_MS);
    }
    ...
    if (playBtn) {
      if (prefersReducedMotion) {
        playBtn.hidden = true;   // nothing auto-advances, nothing to pause
      } else {
        playBtn.addEventListener("click", function () {
          if (holds.has("user")) release("user"); else hold("user");
        });
      }
    }
    if (controls) {
    ...
    // in Chrome and Firefox, and that must not park the slideshow.
    ...
    hero.addEventListener("focusin", function (e) {
      if (isKeyboardFocus(e.target)) hold("focus");
    });
    hero.addEventListener("focusout", function (e) {
      if (!e.relatedTarget || !hero.contains(e.relatedTarget)) release("focus");
    });

    // Hidden tab / locked phone. Checked at init too: a page opened in a
    // background tab must not advance unseen.
    document.addEventListener("visibilitychange", function () {
      if (document.hidden) hold("hidden"); else release("hidden");
    });
    // Back/forward-cache restore: re-assert the DOM from state and make sure
    // a timer is running if it should be.
    window.addEventListener("pageshow", function (e) {
      if (!e.persisted) return;
      render();
      if (!document.hidden) release("hidden");
      if (!timer && canPlay()) arm(remaining > 250 ? remaining : SLIDE_MS);
    });

    // Off screen: no point advancing a hero nobody can see. Also drops the
    // hover hold — a pointer cannot be over something that has scrolled away,
    // and browsers do not always send the leave.
    ...
    /* --- Swipe --- */
    // Horizontal drag past the threshold goes prev/next; no live follow, so
    // the transform transition stays the only motion path. touch-action:
    // pan-y on the track means a vertical gesture scrolls the page natively
    // and cancels this pointer stream.
    var SWIPE_START = 10, SWIPE_COMMIT = 40;
    var drag = null;
    track.addEventListener("pointerdown", function (e) {
      if (e.button !== 0 || drag) return;
      drag = { id: e.pointerId, x: e.clientX, y: e.clientY, moved: false };
      // A mouse drag across the copy would otherwise start a text selection.
      if (e.pointerType === "mouse" && !(e.target.closest && e.target.closest("a, button"))) e.preventDefault();
    });
    track.addEventListener("pointermove", function (e) {
      ...
      if (Math.abs(dx) > SWIPE_START && Math.abs(dx) > Math.abs(dy)) {
        drag.moved = true;
        hold("drag");
        // Keeps pointerup coming to the track even if it ends off the hero,
        // and stops a link under the finger from taking the click.
    ...
    function endDrag(e, commit) {
      ...
      if (!moved) return;
      release("drag");
      if (commit && Math.abs(dx) >= SWIPE_COMMIT) goTo(current + (dx < 0 ? 1 : -1), true);
    }
    track.addEventListener("pointerup", function (e) { endDrag(e, true); });
    track.addEventListener("pointercancel", function (e) { endDrag(e, false); });

    // Arms the slide-entrance styles. Deliberately the last thing before the
    // first render(): the CSS hides slide copy only once this is set, so a
    // failure earlier in this function can never leave the hero blank.
  ...
  function initThemeToggle() {
    ...
    // Safari tints its own chrome (status-bar strip, tab bar) from this meta;
    // solid versions of --color-header-bg so the browser glass and the
    // header plate read as one surface.
    ...
    function sync() {
      ...
      if (themeMeta) themeMeta.setAttribute("content", isDark ? "#090c0e" : "#f7f8f6");
    }
  ...
  function initDroplets() {
    ...
    if (prefersReducedMotion) return;
    // `?off=drops` / `?off=motion` bisect switches (see the diagnostics block
    // at the end of components.css) — build nothing rather than hide it.
    var cl = document.documentElement.classList;
    if (cl.contains("off-drops") || cl.contains("off-motion")) return;
